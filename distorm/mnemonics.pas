...
type
  ...
  _MNEMONICS = array of Byte;
  _REGISTERS = array of _WRegister;

  function GET_REGISTER_NAME(r: WRegister): string;
  function GET_MNEMONIC_NAME(m: WMnemonic): string;
...
function GET_REGISTER_NAME(r: WRegister): string;
begin
  Result := string(PAnsiChar(@r.p[0]));
end;

function GET_MNEMONIC_NAME(m: WMnemonic): string;
begin
  Result := string(PAnsiChar(@m.p[0]));
end;
