...
type
  TDisasmFunc = function(var aDisAsm:TDISASM):longint;stdcall;

var
  hLibHandle: THandle;
  _DisasmFunc: TDisasmFunc;

function Disasm(var aDisAsm:TDISASM):longint;stdcall;
begin
  Result := -1;
  if Assigned(_DisasmFunc) then
    Result := _DisasmFunc(aDisAsm);
end;
...
initialization
  ...
  LibPath := ExtractFilePath(ParamStr(0)) + 'BeaEngine64.dll';
  if not FileExists(LibPath) then
  begin
    R := TResourceStream.Create(HInstance, 'BEA64_IMAGE', RT_RCDATA);
    try
  ...
  if hLibHandle > HINSTANCE_ERROR then
    _DisasmFunc := TDisasmFunc(GetProcAddress(hLibHandle, 'Disasm'));
